...
my $population_size = shift @ARGV;
my $error_rate = shift @ARGV;
...
print STDERR "The output directory for this simulation is: $output_directory \n";

my $simulation_iteration = 100;
my $num_of_round_for_each_player = 200;

#my @strategy_pool_one = ("always_cooperate", "always_defect", "always_trembling", "tit_for_tat", "TFT_with_trembling_hand", "shame_driven_hiding", "shame_driven_denying", "guilt_driven_amending", "Pavlov", "generous_TFT");
my @strategy_pool_one = ("generous_TFT");
my @strategy_pool_two = ("always_cooperate", "always_defect", "always_trembling", "tit_for_tat", "TFT_with_trembling_hand", "shame_driven_hiding", "shame_driven_denying", "guilt_driven_amending", "Pavlov", "generous_TFT");

MAIN_LOOP:foreach my $strategy (@strategy_pool_one){
    
    for (my $i = 0; $i < scalar (@strategy_pool_two); $i++){
        
        if ($strategy eq $strategy_pool_two[$i]){
            
            splice (@strategy_pool_two, $i, 1)
        }
    }
    
    my $loop_controller = scalar(@strategy_pool_two);
    
    if ($loop_controller != 0){
        
        foreach my $opponent_strategy (@strategy_pool_two){
            
            my $strategy_choice_one = $strategy;
            my $strategy_choice_two = $opponent_strategy;
            
            my %num = ( # The number of individuals for each strategy.
                "always_cooperate" => 0,
                "always_defect" => 0,  
                "always_trembling" => 0,
                "tit_for_tat" => 0,
            ...
            my $output_file = $strategy_choice_one."_vs_".$strategy_choice_two;
            ...
            my $header_one = "num_of_".$strategy_choice_one;
            my $header_two = "num_of_".$strategy_choice_two;
            my $header_three = "average_fitness_of_".$strategy_choice_one;
            my $header_four = "average_fitness_of_".$strategy_choice_two;
            
            print STDERR scalar(localtime)."\t".$strategy_choice_one." vs\. ".$strategy_choice_two."\n";
            
            print $OUTPUT_FILE_HANDLE $header_one."\t".$header_two."\t".$header_three."\t".$header_four."\n";
            
            for (my $auto_increment = 0; $auto_increment < ($population_size + 1); $auto_increment++){
                ...
                for (my $j = 0; $j < $simulation_iteration; $j++){ #use $j instead of $i here.
                    ...
                    $num{$strategy_choice_one} = $population_size - $auto_increment;
                    $num{$strategy_choice_two} = 0 + $auto_increment;
                ...
                print $OUTPUT_FILE_HANDLE $num{$strategy_choice_one}."\t".$num{$strategy_choice_two}."\t".$game_average_fitness{$strategy_choice_one}."\t".$game_average_fitness{$strategy_choice_two}."\n";
            }
